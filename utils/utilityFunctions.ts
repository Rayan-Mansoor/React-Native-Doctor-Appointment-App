...
export function extractEmail(sentence: string) : string | null {
...
export function extractPhoneNo(sentence: string): string | null {
  const phoneRegex = /(\+?\d{1,4}[-.\s]?)?(\d{2,4}[-.\s]?\d{3,4}[-.\s]?\d{4,6})/g;
  const matches = sentence.match(phoneRegex);
  if (matches != null) { 
