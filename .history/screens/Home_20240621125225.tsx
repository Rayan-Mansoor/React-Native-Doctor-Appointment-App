...
const Home = ({ navigation }: Props) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      // Update the ref with the latest microphone result when the screen is focused
      microphoneResultRef.current = microphoneResult;

      return () => {
        // Clear the ref when the screen is unfocused
  ...
  useEffect(() => {
    if (microphoneResultRef.current) {
      // Handle the microphone result text
