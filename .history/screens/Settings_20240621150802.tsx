...
const Settings = () => {
  ...
  const handleVoiceCommand = (command: string | null) => {
    ...
    if (command.includes("set font size to")) {
      const newSize = parseInt(command.split("set font size to ")[1], 10);
      handleSizeChange(newSize);
    }
    if (command.includes("set language to english")) {
      handleLanguageChange('en');
    }
    if (command.includes("set language to urdu")) {
      handleLanguageChange('ur');
    }
    if (command.includes("turn on notifications")) {
      setNotificationsEnabled(true);
    }
    if (command.includes("turn off notifications")) {
      setNotificationsEnabled(false);
    }
    if (command.includes("enable colour blindness mode")) {
      handleColorBlindnessToggle();
    }
    if (command.includes("disable colour blindness mode")) {
      handleColorBlindnessToggle();
    }
    if (command.includes("set color blindness mode to protanopia")) {
      handleColorBlindnessChange('protanopia');
    }
    if (command.includes("set color blindness mode to deuteranopia")) {
      handleColorBlindnessChange('deuteranopia');
    }
    if (command.includes("set color blindness mode to tritanopia")) {
