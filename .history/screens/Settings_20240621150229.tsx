...
const Settings = () => {
  ...
  const previousMicrophoneResultRef = useRef<string | null>(null);

  useFocusEffect(
    React.useCallback(() => {
      previousMicrophoneResultRef.current = microphoneResult;
      return () => {
        previousMicrophoneResultRef.current = null;
      };
    }, [microphoneResult])
  );

  useEffect(() => {
    if (microphoneResult && microphoneResult !== previousMicrophoneResultRef.current) {
      previousMicrophoneResultRef.current = microphoneResult;
      handleVoiceCommand(microphoneResult);
      console.log('Microphone Result:', microphoneResult);
    }
  }, [microphoneResult]);
